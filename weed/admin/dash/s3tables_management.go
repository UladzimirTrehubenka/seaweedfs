...
func (s *AdminServer) executeS3TablesOperation(ctx context.Context, operation string, req any, resp any) error {
...
func (s *AdminServer) GetS3TablesBucketsData(ctx context.Context) (S3TablesBucketsData, error) {
	...
	err := s.WithFilerClient(func(client filer_pb.SeaweedFilerClient) error {
		...
		for len(buckets) < s3TablesAdminListLimit {
			...
			if entry.GetEntry() == nil || !entry.GetEntry().GetIsDirectory() {
				continue
			}
			if strings.HasPrefix(entry.GetEntry().GetName(), ".") {
				continue
			}
			metaBytes, ok := entry.GetEntry().GetExtended()[s3tables.ExtendedKeyMetadata]
			if !ok {
				continue
			}
			var metadata tableBucketMetadata
			if err := json.Unmarshal(metaBytes, &metadata); err != nil {
				glog.V(1).Infof("S3Tables: failed to decode table bucket metadata for %s: %v", entry.GetEntry().GetName(), err)

				continue
			}
			arn, err := s3tables.BuildBucketARN(s3tables.DefaultRegion, metadata.OwnerAccountID, entry.GetEntry().GetName())
			if err != nil {
				glog.V(1).Infof("S3Tables: failed to build table bucket ARN for %s: %v", entry.GetEntry().GetName(), err)
...
func (s *AdminServer) SetTableBucketOwner(ctx context.Context, bucketName, owner string) error {
	return s.WithFilerClient(func(client filer_pb.SeaweedFilerClient) error {
		...
		metaBytes, ok := entry.GetExtended()[s3tables.ExtendedKeyMetadata]
		if !ok {
			return fmt.Errorf("table bucket %s metadata missing", bucketName)
		}
		var metadata tableBucketMetadata
