...
func (st *State) Load() error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to read store state from %s : %w", st.FilePath, err)
	}
	if err := proto.Unmarshal(binPb, st.Pb); err != nil {
		st.Pb = nil

		return fmt.Errorf("failed to parse store state from %s : %w", st.FilePath, err)
	}
...
func (st *State) Save() error {
	...
	if err != nil {
		return fmt.Errorf("failed to serialize store state %v: %w", st.Pb, err)
	}
	if err := util.WriteFile(st.FilePath, binPb, StateFileMode); err != nil {
		return fmt.Errorf("failed to write store state to %s : %w", st.FilePath, err)
	}
