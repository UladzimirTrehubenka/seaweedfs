...
func WriteIdxFileFromEcIndex(baseFileName string) (err error) {
	...
	if openErr != nil {
		return fmt.Errorf("cannot open ec index %s.ecx: %w", baseFileName, openErr)
	}
	defer ecxFile.Close()

	idxFile, openErr := os.OpenFile(baseFileName+".idx", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if openErr != nil {
		return fmt.Errorf("cannot open %s.idx: %w", baseFileName, openErr)
	}
	defer idxFile.Close()
...
func FindDatFileSize(dataBaseFileName, indexBaseFileName string) (datSize int64, err error) {
	...
	if err != nil {
		return 0, fmt.Errorf("read ec volume %s version: %w", dataBaseFileName, err)
	}
...
func readEcVolumeVersion(baseFileName string) (version needle.Version, err error) {
	...
	if err != nil {
		return 0, fmt.Errorf("open ec volume %s superblock: %w", baseFileName, err)
	}
	...
	datBackend.Close()
	if err != nil {
		return 0, fmt.Errorf("read ec volume %s superblock: %w", baseFileName, err)
	}
...
func iterateEcjFile(baseFileName string, processNeedleFn func(key types.NeedleId) error) error {
	...
	if openErr != nil {
		return fmt.Errorf("cannot open ec index %s.ecj: %w", baseFileName, openErr)
	}
	defer ecjFile.Close()
...
func WriteDatFile(baseFileName string, datFileSize int64, shardFileNames []string) error {
	...
	if openErr != nil {
		return fmt.Errorf("cannot write volume %s.dat: %w", baseFileName, openErr)
	}
	defer datFile.Close()
	...
	defer func() {
		for shardId := range DataShardsCount {
	...
	for datFileSize >= DataShardsCount*ErasureCodingLargeBlockSize {
		for shardId := range DataShardsCount {
			...
			if w != ErasureCodingLargeBlockSize {
				return fmt.Errorf("copy %s large block on shardId %d: %w", baseFileName, shardId, err)
			}
	...
	for datFileSize > 0 {
		for shardId := range DataShardsCount {
			...
			if w != toRead {
				return fmt.Errorf("copy %s small block %d: %w", baseFileName, shardId, err)
			}
			datFileSize -= toRead
		}
	}
