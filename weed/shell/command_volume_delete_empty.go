	...
	eachDataNode(topologyInfo, func(dc DataCenterId, rack RackId, dn *master_pb.DataNodeInfo) {
		for _, diskInfo := range dn.GetDiskInfos() {
			for _, v := range diskInfo.GetVolumeInfos() {
				if v.GetSize() <= super_block.SuperBlockSize && v.GetModifiedAtSecond() > 0 && v.GetModifiedAtSecond()+quietSeconds < nowUnixSeconds {
					if *applyBalancing {
						log.Printf("deleting empty volume %d from %s", v.GetId(), dn.GetId())
						if deleteErr := deleteVolume(commandEnv.option.GrpcDialOption, needle.VolumeId(v.GetId()),
						...
						log.Printf("empty volume %d from %s", v.GetId(), dn.GetId())
					}
				}
			}
		}
