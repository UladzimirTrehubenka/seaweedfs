...
func diskInfosToString(diskInfos map[string]*master_pb.DiskInfo) string {
	...
	for _, diskType := range sortMapKey(diskInfos) {
		...
		if diskType == "" {
			diskType = types.HddType.String()
		}
		fmt.Fprintf(&buf, " %s(volume:%d/%d active:%d free:%d remote:%d)", diskType, diskInfo.GetVolumeCount(), diskInfo.GetMaxVolumeCount(), diskInfo.GetActiveVolumeCount(), diskInfo.GetFreeVolumeCount(), diskInfo.GetRemoteVolumeCount())
	}
...
func diskInfoToString(diskInfo *master_pb.DiskInfo) string {
	var buf bytes.Buffer
	fmt.Fprintf(&buf, "volume:%d/%d active:%d free:%d remote:%d", diskInfo.GetVolumeCount(), diskInfo.GetMaxVolumeCount(), diskInfo.GetActiveVolumeCount(), diskInfo.GetFreeVolumeCount(), diskInfo.GetRemoteVolumeCount())
...
func (c *commandVolumeList) writeTopologyInfo(writer io.Writer, t *master_pb.TopologyInfo, volumeSizeLimitMb uint64, verbosityLevel int) *statistics {
	output(verbosityLevel >= 0, writer, "Topology volumeSizeLimit:%d MB%s\n", volumeSizeLimitMb, diskInfosToString(t.GetDiskInfos()))
	slices.SortFunc(t.GetDataCenterInfos(), func(a, b *master_pb.DataCenterInfo) int {
		return strings.Compare(a.GetId(), b.GetId())
	})
	s := newStatistics()
	for _, dc := range t.GetDataCenterInfos() {
		if *c.dataCenter != "" && *c.dataCenter != dc.GetId() {
			continue
		}
...
func (c *commandVolumeList) writeDataCenterInfo(writer io.Writer, t *master_pb.DataCenterInfo, verbosityLevel int) statistics {
	var s statistics
	slices.SortFunc(t.GetRackInfos(), func(a, b *master_pb.RackInfo) int {
		return strings.Compare(a.GetId(), b.GetId())
	})
	dataCenterInfoFound := false
	for _, r := range t.GetRackInfos() {
		if *c.rack != "" && *c.rack != r.GetId() {
			continue
		}
		s.add(c.writeRackInfo(writer, r, verbosityLevel, func() {
			output(verbosityLevel >= 1, writer, "  DataCenter %s%s\n", t.GetId(), diskInfosToString(t.GetDiskInfos()))
		}))
...
func (c *commandVolumeList) writeRackInfo(writer io.Writer, t *master_pb.RackInfo, verbosityLevel int, outCenterInfo func()) statistics {
	var s statistics
	slices.SortFunc(t.GetDataNodeInfos(), func(a, b *master_pb.DataNodeInfo) int {
		return strings.Compare(a.GetId(), b.GetId())
	})
	rackInfoFound := false
	for _, dn := range t.GetDataNodeInfos() {
		if *c.dataNode != "" && *c.dataNode != dn.GetId() {
			continue
		}
...
func (c *commandVolumeList) writeDataNodeInfo(writer io.Writer, t *master_pb.DataNodeInfo, verbosityLevel int, outRackInfo func()) statistics {
	var s statistics
	diskInfoFound := false
	for _, diskType := range sortMapKey(t.GetDiskInfos()) {
		diskInfo := t.GetDiskInfos()[diskType]
...
func (c *commandVolumeList) writeDiskInfo(writer io.Writer, t *master_pb.DiskInfo, verbosityLevel int, outNodeInfo func()) statistics {
	...
	slices.SortFunc(t.GetVolumeInfos(), func(a, b *master_pb.VolumeInformationMessage) int {
		return int(a.GetId()) - int(b.GetId())
	})
	volumeInfosFound := false

	for _, vi := range t.GetVolumeInfos() {
		if c.isNotMatchDiskInfo(vi.GetReadOnly(), vi.GetCollection(), vi.GetId(), int64(vi.GetSize())) {
			continue
		}
		if !volumeInfosFound {
			outNodeInfo()
			output(verbosityLevel >= 4, writer, "        Disk %s(%s) id:%d\n", diskType, diskInfoToString(t), t.GetDiskId())
			volumeInfosFound = true
		}
		s.add(writeVolumeInformationMessage(writer, vi, verbosityLevel))
	}
	ecShardInfoFound := false
	for _, ecShardInfo := range t.GetEcShardInfos() {
		if c.isNotMatchDiskInfo(false, ecShardInfo.GetCollection(), ecShardInfo.GetId(), -1) {
			continue
		}
...
func writeVolumeInformationMessage(writer io.Writer, t *master_pb.VolumeInformationMessage, verbosityLevel int) statistics {
	...
	return statistics{
		...
		DeletedFileCount: t.GetDeleteCount(),
		DeletedBytes:     t.GetDeletedByteCount(),
	}
}
...
func writeECShardInformationMessage(writer io.Writer, t *master_pb.VolumeEcShardInformationMessage, verbosityLevel int) statistics {
	...
	if t.GetExpireAtSec() > 0 {
		expireAtString = "expireAt:" + time.Unix(int64(t.GetExpireAtSec()), 0).Format("2006-01-02 15:04:05")
	}
	...
	output(verbosityLevel >= 5, writer, "          ec volume id:%v collection:%v shards:%v%s %s\n",
		t.GetId(), t.GetCollection(), si.Ids(), shardSizeInfo, expireAtString)
...
func output(condition bool, w io.Writer, format string, a ...any) {
	if condition {
