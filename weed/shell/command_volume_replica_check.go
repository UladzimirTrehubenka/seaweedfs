...
func getVolumeReplicaStatus(grpcDialOption grpc.DialOption, vid needle.VolumeId, location wdclient.Location) VolumeReplicaStatus {
	...
	err := operation.WithVolumeServerClient(false, location.ServerAddress(), grpcDialOption, func(volumeServerClient volume_server_pb.VolumeServerClient) error {
		...
		if resp != nil {
			status.FileCount = resp.GetFileCount()
			status.FileDeletedCount = resp.GetFileDeletedCount()
			status.VolumeSize = resp.GetVolumeSize()
			status.IsReadOnly = resp.GetIsReadOnly()
		}
...
func (rub *replicaUnionBuilder) buildUnionReplica(locations []wdclient.Location, excludeFromSelection string) (wdclient.Location, int, error) {
	if len(locations) == 0 {
		return wdclient.Location{}, 0, errors.New("no replicas available")
	}
	if len(locations) == 1 {
		if locations[0].Url == excludeFromSelection {
			return wdclient.Location{}, 0, errors.New("only replica is excluded")
		}
	...
	if bestIdx == -1 {
		return wdclient.Location{}, 0, errors.New("could not find valid replica (all excluded or errored)")
	}
	...
	if bestDB == nil {
		return wdclient.Location{}, 0, errors.New("failed to allocate in-memory needle DB")
	}
	defer bestDB.Close()
