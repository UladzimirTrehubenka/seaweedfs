...
func (b *MessageQueueBroker) SubscribeMessage(stream mq_pb.SeaweedMessaging_SubscribeMessageServer) error {
	...
	clientName := fmt.Sprintf("%s/%s-%s", req.GetInit().GetConsumerGroup(), req.GetInit().GetConsumerId(), req.GetInit().GetClientId())

	t := topic.FromPbTopic(req.GetInit().GetTopic())
	partition := topic.FromPbPartition(req.GetInit().GetPartitionOffset().GetPartition())

	glog.V(0).Infof("Subscriber %s on %v %v connected", req.GetInit().GetConsumerId(), t, partition)
	...
	imt := sub_coordinator.NewInflightMessageTracker(int(req.GetInit().GetSlidingWindowSize()))
	...
	glog.V(0).Infof("follower broker: %v", req.GetInit().GetFollowerBroker())
	if req.GetInit().GetFollowerBroker() != "" {
		follower := req.GetInit().GetFollowerBroker()
		if followerGrpcConnection, err := pb.GrpcDial(ctx, follower, true, b.grpcDialOption); err != nil {
			return fmt.Errorf("fail to dial %s: %w", follower, err)
		} else {
			defer func() {
			...
			if subscribeFollowMeStream, err = followerClient.SubscribeFollowMe(ctx); err != nil {
				return fmt.Errorf("fail to subscribe to %s: %w", follower, err)
			} else {
				if err := subscribeFollowMeStream.Send(&mq_pb.SubscribeFollowMeRequest{
					Message: &mq_pb.SubscribeFollowMeRequest_Init{
						Init: &mq_pb.SubscribeFollowMeRequest_InitMessage{
							Topic:         req.GetInit().GetTopic(),
							Partition:     req.GetInit().GetPartitionOffset().GetPartition(),
							ConsumerGroup: req.GetInit().GetConsumerGroup(),
						},
					},
				}); err != nil {
					return fmt.Errorf("fail to send init to %s: %w", follower, err)
				}
			}
		}
	...
	go func() {
		...
		for {
			...
			if seekMsg := ack.GetSeek(); seekMsg != nil {
				glog.V(0).Infof("Subscriber %s received seek request to offset %d (type %v)",
					clientName, seekMsg.GetOffset(), seekMsg.GetOffsetType())
			...
			imt.AcknowledgeMessage(ack.GetAck().GetKey(), ack.GetAck().GetTsNs())
		...
		if lastOffset > 0 {
			glog.V(0).Infof("saveConsumerGroupOffset %v %v %v %v", t, partition, req.GetInit().GetConsumerGroup(), lastOffset)
			if err := b.saveConsumerGroupOffset(t, partition, req.GetInit().GetConsumerGroup(), lastOffset); err != nil {
	...
	for {
		...
		go func() {
			subscribeErr := localTopicPartition.Subscribe(clientName, currentPosition, func() bool {
				...
				if logEntry.Key != nil {
					imt.EnflightMessage(logEntry.GetKey(), logEntry.GetTsNs())
				}
				...
				subscriber.UpdateReceivedOffset(logEntry.GetTsNs())
		...
		case seekMsg := <-seekChan:
			...
			glog.V(0).Infof("Subscriber %s seeking from offset %d to offset %d (type %v)",
				clientName, currentPosition.GetOffset(), seekMsg.GetOffset(), seekMsg.GetOffsetType())
			...
			glog.V(0).Infof("Subscriber %s restarting Subscribe from new offset %d", clientName, seekMsg.GetOffset())
...
func (b *MessageQueueBroker) getRequestPosition(initMessage *mq_pb.SubscribeMessageRequest_InitMessage) (startPosition log_buffer.MessagePosition) {
	...
	offsetType := initMessage.GetOffsetType()
	...
	if offsetType == schema_pb.OffsetType_EXACT_TS_NS {
		startPosition = log_buffer.NewMessagePosition(offset.GetStartTsNs(), -2)
	...
	if offsetType == schema_pb.OffsetType_EXACT_OFFSET {
		startPosition = log_buffer.NewMessagePositionFromOffset(offset.GetStartOffset())
	...
	if storedOffset, err := b.readConsumerGroupOffset(initMessage); err == nil {
		glog.V(0).Infof("resume from saved offset %v %v %v: %v", initMessage.GetTopic(), initMessage.GetPartitionOffset().GetPartition(), initMessage.GetConsumerGroup(), storedOffset)
...
func (b *MessageQueueBroker) getRequestPositionFromSeek(seekMsg *mq_pb.SubscribeMessageRequest_SeekMessage) (startPosition log_buffer.MessagePosition) {
	...
	offsetType := seekMsg.GetOffsetType()
	offset := seekMsg.GetOffset()
