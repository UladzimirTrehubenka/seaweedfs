...
func (f *Filer) MaybeBootstrapFromOnePeer(self pb.ServerAddress, existingNodes []*master_pb.ClusterNodeUpdate, snapshotTime time.Time) (err error) {
	...
	sort.Slice(existingNodes, func(i, j int) bool {
		return existingNodes[i].GetCreatedAtNs() < existingNodes[j].GetCreatedAtNs()
	})
	earliestNode := existingNodes[0]
	if earliestNode.GetAddress() == string(self) {
		return
	}

	glog.V(0).Infof("bootstrap from %v clientId:%d", earliestNode.GetAddress(), f.UniqueFilerId)

	return pb.WithFilerClient(false, f.UniqueFilerId, pb.ServerAddress(earliestNode.GetAddress()), f.GrpcDialOption, func(client filer_pb.SeaweedFilerClient) error {
...
func (f *Filer) AggregateFromPeers(self pb.ServerAddress, existingNodes []*master_pb.ClusterNodeUpdate, startFrom time.Time) {
	...
	for _, node := range existingNodes {
		address := pb.ServerAddress(node.GetAddress())
...
func (f *Filer) setOrLoadFilerStoreSignature(store FilerStore) (isFresh bool) {
	...
	if errors.Is(err, ErrKvNotFound) || err == nil && len(storeIdBytes) == 0 {
...
func (f *Filer) CreateEntry(ctx context.Context, entry *Entry, o_excl bool, isFromOtherCluster bool, signatures []int32, skipCreateParentDir bool, maxFilenameLength uint32) error {
	...
	if entry.IsLongerFileName(maxFilenameLength) {
		return errors.New("entry name too long")
	}
	...
	*/
	...
	if oldEntry == nil {
		...
		if err := f.Store.InsertEntry(ctx, entry); err != nil {
			...
			return fmt.Errorf("insert entry %s: %w", entry.FullPath, err)
		}
	} else {
		if o_excl {
	...
	if dirEntry == nil {
		...
		if isUnderBuckets {
			if err := s3bucket.VerifyS3BucketName(dirParts[2]); err != nil {
				return fmt.Errorf("invalid bucket name %s: %w", dirParts[2], err)
			}
		}
		...
		if mkdirErr != nil {
			if fEntry, err := f.FindEntry(ctx, util.FullPath(dirPath)); errors.Is(err, filer_pb.ErrNotFound) || fEntry == nil {
